...
function Client(client_name) {
    ...
    this.debug = 0; //debug level, 0-5 (5 will output extremely lot of data)
    ...
    this.spawn_attempt = 1; //attempt to spawn
...
Client.prototype = {
    ...
    onConnect: function() {
        ...
        if(this.origin == "http://mgar.io") {
            var b = new Uint8Array([254, 5, 0, 0, 0]);
            var b = new Uint8Array([255, 109, 103, 97, 114]);   
          this.send(b)
        }
        if(this.origin == "http://rata.io") {
            var b = new Uint8Array([254, 0, 0, 0, 0]);
            var b = new Uint8Array([255, 0, 0, 0, 0]);
           var buf = new Buffer(5);
            buf.writeUInt8(254, 0);
            buf.writeUInt32LE(5, 1);
          this.send(buf)
        ...
        if(this.origin == "http://cellcraft.io") {
            var b = new Uint8Array([255, 50, 137, 112, 79]);
            var b = new Uint8Array([42]);
          this.send(b)
        }
      if(this.origin == "http://galx.io") {
            var b = new Uint8Array([255, 50, 137, 112, 79]);
            var b = new Uint8Array([43]);
            var b = new Uint8Array([43]);
            var b = new Uint8Array([46]);
            var b = new Uint8Array([45]);
        this.send(b)
      }
      if (this.origin == "http://gota.io") {
        var b = new Uint8Array([255, 71, 111, 116, 97, 32, 87, 101, 98, 32, 49, 46, 50, 46, 48, 0]);
        var b = new Uint8Array([10, 0, 255, 255, 0, 0, 0, 0]);
        var b = new Uint8Array([71]);
        this.send(b)
      }
              if(this.origin == "http://agma.io") {
            var b = new Uint8Array([255, 50, 137, 112, 79]);
            var b = new Uint8Array([43]);
            var b = new Uint8Array([43]);
            var b = new Uint8Array([46]);
    ...
    spawn: function(name) {
        if(this.debug >= 3)
            console.log('spawn() called, name=' + name);

        if(!this.ws || this.ws.readyState !== WebSocket.OPEN) {
            if(this.debug >= 1)
                console.log('[warning] spawn() was called when connection was not established, packet will be dropped');
            return false;
        }
    ...
    moveTo: function(x, y, l, p, c) {
        ...
        if(c) {
            ...
            buf.writeUInt16LE(Math.random() * 65535, 2);
            this.send(buf);
        }
